...
void CalculatorAimAngle(Vector* MyPos, Vector* EntityPos);
...
void doAimBot()
{
	if (Controller::AimBot && GetAsyncKeyState(ALT_KEY) && true && Interface)
	{
		...
		if (TargetLoop > 0)
		{
			...
			Vector MyPos = LocalPlayer->GetPos();
			Vector EntityPos = AimDistance->GetPlayer3DPos();
			CalculatorAimAngle(&MyPos, &EntityPos);
		}
