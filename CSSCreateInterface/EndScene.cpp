...
HRESULT WINAPI Hooks::Present(LPDIRECT3DDEVICE9 Device, const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion)
{
	/*if (bInit == false)
	{
		bInit = true;
	...
	Tools::DrawString(800 + 1, 50 + 1, 0, 0, 0, "The Present Hook is successful.");
	Tools::DrawString(800 + 1, 50 - 1, 0, 0, 0, "The Present Hook is successful.");
	Tools::DrawString(800 - 1, 50 - 1, 0, 0, 0, "The Present Hook is successful.");
	Tools::DrawString(800 - 1, 50 + 1, 0, 0, 0, "The Present Hook is successful.");
	Tools::DrawString(800, 50, 200, 200, 200, "The Present Hook is successful.");*/
	...
	if (Controller::windowVisible)
	{
		...
		ImGui::Begin(" Counter-Strike:Source - SDK GameHacking");
		{
			...
			if (ImGui::CollapsingHeader("ESP-TargitLine"))//目標線條
			{
			...
			if (ImGui::CollapsingHeader("ESP-BonesESP"))///骨骼透視
			{
			...
			if (ImGui::CollapsingHeader("ESP-BOX"))//ESP BOX 透視
			{
				ImGui::Checkbox("BOX", &Controller::ESPBOX);//ESP BOX 透視
			...
			if (ImGui::CollapsingHeader("ESP-Information"))//玩家資訊
			{
				ImGui::Checkbox("Information", &Controller::EspPlayerInfo);//玩家資訊
	...
	//delete ClassId;//關閉ClassId 防止記憶體外洩
